-- ================================================================
-- FIX: Corrigir função search_available_slots
-- Problema: generate_series não funciona diretamente com TIME
-- Solução: Usar TIMESTAMP e converter para TIME
-- ================================================================
...
BEGIN
    ...
    eligible_therapists AS (
        SELECT
            ...
            CASE
                WHEN p_discipline_id IS NULL THEN true
                WHEN ts.discipline_id IS NOT NULL THEN true
        ...
        LEFT JOIN therapist_specialties ts
            ...
            AND ts.discipline_id = p_discipline_id
    ...
    -- 3. Slots de tempo (intervalos de 30 min) - CORRIGIDO
    time_slots AS (
        SELECT
            (ts_timestamp::TIME) as slot_time
        FROM generate_series(
            ('2000-01-01 ' || v_start_time)::TIMESTAMP,
            ('2000-01-01 ' || v_end_time)::TIMESTAMP - (p_duration_minutes || ' minutes')::INTERVAL,
            '30 minutes'::INTERVAL
        ) ts_timestamp
    ),
...
COMMENT ON FUNCTION search_available_slots IS 'Busca horários disponíveis para agendamento com filtros avançados';
