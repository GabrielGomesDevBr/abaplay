-- ================================================================
-- MIGRATION 010: Função de Busca de Disponibilidade
-- Data: Janeiro 2025
-- Descrição: Função principal para buscar horários disponíveis
-- ================================================================
...
BEGIN
    ...
    time_slots AS (
        SELECT
            generate_series(
                v_start_time,
                v_end_time - (p_duration_minutes || ' minutes')::INTERVAL,
                '30 minutes'::INTERVAL
            )::TIME as slot_time
    ),
    ...
    potential_slots AS (
        SELECT
            ...
            TO_CHAR(dtc.check_date, 'TMDay') as day_name
